...
# Filename       :    res32_bilstm_ctc.py
# Abstract       :    Base recognition Model, res32 bilstm ctc
...
model = dict(
    ...
    sequence_head=dict(                 # Recognition head parameter
        ...
        loss_ctc=dict(                  # Model loss function
            type='WarpCTCLoss',
            blank=0,                    # when use_cha_eos=True:96 else:0
            length_average=True,
            size_average=True,
            loss_weight=1.0))
)
...
data_types=['LMDB','File','Tight','File']     # corresponding to different
...
train_pipelines = [
    dict(
        ...
        fil_ops=True,                  # whether to filter unsupported text
    ),
    dict(
